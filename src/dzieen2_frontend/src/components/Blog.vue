<template>
    <div>
        ...
        <div class="flex justify-center flex-col">
            ...
            <button class="bg-blue-600 rounded-md text-white p-4" @click="dodajWpisy">dodaj</button>
        </div>
    </div>
</template>

<script>
import { dzieen2_backend } from 'declarations/dzieen2_backend/index';
...
export default {
    ...
    methods: {
        async dodajWpisy() {
            await dzieen2_backend.dodaj_wpis(this.nowyBlog);
            await this.pobierzWpisy();
        },
        async deleteWpis(index) {
            await dzieen2_backend.usun_wpis(index);
            await this.pobierzWpisy();
        },
        async pobierzWpisy() {
            this.wpisy = await dzieen2_backend.odczytaj_wpisy();
        }
    },
    async mounted(){
        this.pobierzWpisy()
